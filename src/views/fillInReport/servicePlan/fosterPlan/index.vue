  ...
  <div v-loading.lock="fullscreenLoading">
    ...
    <div class="page-content" ref="pageContent">
      ...
      <vxe-table
        ...
        <vxe-table-column field="fcustomername" fixed="left" min-width="80" title="养户姓名">
          <template v-slot="{row}">
            {{row.fcustomername || '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="fdevelopername" min-width="80" title="开发员">
          <template v-slot="{row}">
            {{row.fdevelopername || '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="fstaockscale" min-width="80" title="存栏规模" align="right">
          <template v-slot="{row}">
            {{row.fstaockscale || '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="faddress" min-width="80" title="猪舍地址">
          <template v-slot="{row}">
            {{row.faddress || '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="fpiggerytype" min-width="80" title="猪舍新建/改建">
          <template v-slot="{row}">
            {{row.fpiggerytype || '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="fplandate" min-width="80" title="预缴保证金日期">
          <template v-slot="{ row }">{{ row.fplandate ? formatDate(row.fplandate, 'yyyy-MM-dd') : '-' }}</template>
        </vxe-table-column>
        <vxe-table-column field="fishandcusloan" min-width="60" title="是否自愿办养户贷">
          <template v-slot="{ row }">{{ row.fishandcusloan ? enumBoolean(row.fishandcusloan) : '-' }}</template>
        </vxe-table-column>
        <vxe-table-column field="fhandcusloandate" min-width="80" title="办理养户贷日期">
          <template v-slot="{ row }">{{ row.fhandcusloandate ? formatDate(row.fhandcusloandate, 'yyyy-MM-dd') : '-'}}
          </template>
        </vxe-table-column>
        <vxe-table-column field="fissigncooper" min-width="60" title="是否签订合作意向书">
          <template v-slot="{ row }">{{ row.fissigncooper ? enumBoolean(row.fissigncooper) : '-'}}</template>
        </vxe-table-column>
      </vxe-table>
...
<script>
  ...
  import {
    getFosterPlanList,
    getFosterPlanListDetails,
    toExamine,
    backToExamine
  } from '@/api/serviceDepartment/fosterPlan'
    ...
    methods: {
        ...
        getFosterPlanList({
          ...
          if (res.status === 200) {
            if (res.data.length) {
              ...
              let childTableObj = this.parentData[0].ctPlaYzrhMonplEntryList
      ...
      exportChildData() {  // 导入数据
        if (this.fparentid) {
          let data = {
            method: 'get',
            url: `CtPlaYzrhMonpl/exportExcel/${this.fparentid}`,
            fileName: '养户开发计划表',
          };
