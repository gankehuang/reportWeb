  ...
  <div v-loading.fullscreen.lock="loading">
    ...
    <div class="page-content" ref="pageContent">
        ...
        <vxe-toolbar>
          ...
          <template v-slot:tools>
            ...
            <!--							<el-button size="small"  @click="importDataEvent" v-if="editable" icon="el-icon-download" class="el-button-submit">导入</el-button>-->
              ...
              action="/taskTrackEntry/importExcelByList"
              accept=".xls,.xlsx"
      ...
      <vxe-table fit align="left" header-align="left" highlight-hover-row border
                 ...
                 :edit-config="editable?{trigger: 'click', mode: 'row',showStatus: true}:{}"
                 :data="mode?mode.taskTrackEntry:[]">
        <vxe-table-column v-if="editable" type="checkbox" align="center" width="60" fixed="left"></vxe-table-column>
        <vxe-table-column title="任务名称"  field="taskName" :edit-render="{name: 'input', props: {placeholder: '请输入值'}}"
                      show-overflow="false"   width="180" fixed="left"></vxe-table-column>
        <vxe-table-column title="任务内容描述" field="taskContent"
                       show-overflow="false"   :edit-render="{name: 'input', props: {placeholder: '请输入值'}}" min-width="100"></vxe-table-column>
        <vxe-table-column title="任务来源" field="taskSource" :edit-render="{name: 'input', props: {placeholder: '请输入值'}}"
                       show-overflow="false" width="100"  ></vxe-table-column>
        <vxe-table-column title="要求交付结果" field="askResults" :edit-render="{name: 'input', props: {placeholder: '请输入值'}}"
                       show-overflow="false"  width="100" ></vxe-table-column>
        <vxe-table-column title="要求达成目标" field="askGoal" :edit-render="{name: 'input', props: {placeholder: '请输入值'}}"
                       show-overflow="false" width="100" ></vxe-table-column>
        <vxe-table-column title="发布时间" field="publishTime" :edit-render="{name: 'input'}" width="100" show-overflow="false" >
          <template v-slot:edit="{ row }">
            <el-date-picker v-model="row.publishTime" type="date" format="yyyy-MM-dd" value-format="timestamp"
        ...
        <vxe-table-column title="计划完成时间" field="planTime" :edit-render="{type: 'default'}" width="100" show-overflow="false" >
          <template v-slot:edit="{ row }">
            <el-date-picker v-model="row.planTime" type="date" format="yyyy-MM-dd" value-format="timestamp"
                            style="width: 100%;"></el-date-picker>
          </template>
          <template v-slot="{ row }">{{ formatDate(row.planTime,'yyyy-MM-dd')}}</template>
        </vxe-table-column>
        <vxe-table-column title="责任人" field="ownersName" :edit-render="{name: 'input', props: {placeholder: '请输入键值'}}"
                         show-overflow="false" width="100"></vxe-table-column>
      </vxe-table>
    </div>
  </div>
...
<script>
    ...
    import taskbillHttp from "@/api/businessUnit/taskbill/index"
    ...
    export default {
        ...
        methods: {
            ...
            approve(status,message) {
                ...
                _self.loading = true
                taskbillHttp.approve(req).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        this.$message({
                            message: message+'成功',
            ...
            save(status,message) {
                ...
                if (!parentItem.taskTrackEntry || parentItem.taskTrackEntry.length == 0) {
                    this.$message({
                        message: '无任务信息,请添加',
                        type: 'error'
                    });
                ...
                _self.loading = true
                taskbillHttp.add(parentItem).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        this.$message({
                            message: message+'成功',
            ...
            deleteInfo() {
                let _self = this;
                _self.loading = true
                taskbillHttp.deleteInfo(this.mode.id).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        this.$message({
                            message: '删除成功',
...
//				let insertRecords = this.$refs.childxTable.getInsertRecords() // 获取新增
                ...
                let taskTrackEntry = data.concat(removeRecords);

                parentItem.taskTrackEntry = taskTrackEntry;
                return parentItem;
            },
            addLine() { // 增加一行
                let record = {}
...
//					this.showChart()
            ...
            exportMethod(commitData){
              axios({
                method: 'post',
                url: `${process.env.BASE_API}/BUSINESSREPORT/taskTrackEntry/exportExcelByList`,
                responseType: 'blob',
                timeout: 1000 * 30,
                data: commitData,
                headers: {
                  ...
                  link.href = url;
                  link.setAttribute('download', '任务清单' + '.xlsx');
            ...
            importDataEvent(params) {
                // this.$refs.childxTable.importData()
                 ...
                 taskbillHttp.importReport(formData).then(res => {
                   this.$refs.upload.clearFiles();
                   if (res.status === 200) {
