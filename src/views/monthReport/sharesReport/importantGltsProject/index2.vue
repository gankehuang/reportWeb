    ...
    methods: {
      ...
      editTemplate(){
        if (!this.selectionId) {
          ...
          this.$router.push({name: "importantGltsProjectadd",query: {id: this.selectionId,status:this.status,isLook:false}})
        }
      },
      audit(data) {
        if (data.taskId) {
          this.$router.push({
            name: "importantGltsProjectadd",
            query: {
              id: data.id,
              workflow: 1,
              status: data.status,
      ...
      addTemplate(){
        this.$router.push("importantGltsProjectadd")
      },
