  ...
  <div v-loading.fullscreen.lock="loading">
    ...
    <div class="page-content" ref="pageContent">
        ...
        <vxe-toolbar>
          ...
          <template v-slot:tools>
            ...
            v-if="editable"
             ...
             action="/projectAnalysisEntry/importExcelByList"
             accept=".xls,.xlsx"
      ...
      <vxe-table fit align="left" header-align="left" highlight-hover-row border
                 ...
                 :data="mode?mode.projectAnalysisEntrys:[]">
        ...
        <vxe-table-column title="项目名称" field="projectName" :edit-render="{name: 'input'}"
                          fixed="left" show-overflow="false" width="180"></vxe-table-column>
        <vxe-table-column title="项目主要内容" field="projectContent" :edit-render="{name: 'input'}"
                           show-overflow="false" width="100"></vxe-table-column>

        <vxe-table-column title="项目规模" field="projectScale" :edit-render="{name: 'input'}"
                           show-overflow="false" width="100"></vxe-table-column>
        <vxe-table-column title="预期交付结果" field="expectResults" :edit-render="{name: 'input'}"
                           show-overflow="false" width="100"></vxe-table-column>
        <vxe-table-column title="预期达成目标" field="expectGoal" :edit-render="{name: 'input'}"
                           show-overflow="false" width="100"></vxe-table-column>
        ...
        <vxe-table-column title="上月达成情况" field="lastMonthCondition" :edit-render="{name: 'input'}"
                           show-overflow="false" width="100">
          <template #header>
        		{{ monthText(-1) + '达成情况' }}
        	</template>
        ...
        <vxe-table-column title="责任人" field="ownersName" :edit-render="{name: 'input'}"  show-overflow="false" width="100"></vxe-table-column>
...
<script>
    ...
    import importantTzProjectHttp from "@/api/businessUnit/importantTzProject/index"
...
//            this.$XModal.message({ status: 'error', message: '校验不通过！' })
            ...
            approve(status,message) {
                ...
                _self.loading = true
                importantTzProjectHttp.approve(req).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        this.$message({
                            message: message+'成功',
            ...
            save(status,message) {
                ...
                if (!parentItem.projectAnalysisEntrys || parentItem.projectAnalysisEntrys.length == 0) {
                    this.$message({
                        message: '无项目信息,请添加',
                        type: 'error'
                    });
                ...
                _self.loading = true
                importantTzProjectHttp.add(parentItem).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        _self.find();
                        this.$message({
            ...
            find() {
                ...
                _self.loading = true
                importantTzProjectHttp.find(req)
                    .then((res) => {
                        _self.loading = false
                        if (res.data) {
                            _self.mode = res.data
            ...
            deleteInfo() {
                let _self = this
                _self.loading = true
                importantTzProjectHttp.deleteInfo(this.mode.id).then((res) => {
                    _self.loading = false
                    if (res.status == 200) {
                        this.$message({
                            message: '删除成功',
            ...
            getFormData(btnStatus) {
                ...
                let removeRecords = this.$refs.childxTable.getRemoveRecords() // 获取删除
...
//
                let projectAnalysisEntrys = data.concat(removeRecords);

                parentItem.projectAnalysisEntrys = projectAnalysisEntrys;
                return parentItem;
            },
            addLine() { // 增加一行
                let record = {}
            ...
            exportMethod(commitData){
              axios({
                method: 'post',
                url: `${process.env.BASE_API}/BUSINESSREPORT/projectAnalysisEntry/exportExcelByList`,
                responseType: 'blob',
                timeout: 1000 * 30,
                data: commitData,
                headers: {
                  ...
                  link.href = url;
                  link.setAttribute('download', '重要投资项目' + '.xlsx');
            ...
            importDataEvent(params) {
                // this.$refs.childxTable.importData()
                 ...
                 importantTzProjectHttp.importReport(formData).then(res => {
                   this.$refs.upload.clearFiles();
                   if (res.status === 200) {
