...
<!--          <el-button size="small" :disabled="loading" v-show="formData.status === 0"  @click="refreshBusiness" class="el-button-audit">刷新经营业绩</el-button>-->
        ...
        <vxe-table
          ...
          border
          :height="autoHeight">
          <vxe-table-column title="项目" align="center" width="40"
                            field="quotaBigType"></vxe-table-column>
          <vxe-table-column title="项目" align="center" width="40" field="quotaSmallType"></vxe-table-column>
          <vxe-table-column title="指标" :className="kpiClassRender" width="200" field="quotaName">
            <template v-slot="{ row }">
              <el-link type="primary" v-if="deepKpiKeys.includes(row.quotaId)" @click="deepJump(row)">{{ row.quotaName }}
              </el-link>
              <span v-else>{{ row.quotaName || '' }}</span>
            </template>
          </vxe-table-column>
            <vxe-table-column :title="monthText(0) + '计划'" className="ch-bg-gary2" align="right" width="80" field="lastTarget">
              <template v-slot="{ row }">
                {{ row.lastTarget || '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column :title="monthText(0) + '累计'" className="ch-bg-green" align="right" field="lastComplete" width="100">
              <template v-slot="{ row }">
          ...
          <vxe-table-column :title="monthText(0) + '计划'" width="80" align="center">
            <vxe-table-column title="1-7日计划"  align="right" width="70" field="firstWeekTarget">
              <template v-slot="{ row }">
                {{ row.firstWeekTarget ? kpiFormatter(row.firstWeekTarget,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="1-7日实际"  align="right" width="70" field="firstWeekFact">
              <template v-slot="{ row }">
                {{ row.firstWeekFact ? kpiFormatter(row.firstWeekFact,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="8-14日计划" align="right" width="80" field="secondWeekTarget">
              <template v-slot="{ row }">
                {{ row.secondWeekTarget ? kpiFormatter(row.secondWeekTarget,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="8-14日实际" align="right" width="80" field="secondWeekFact">
              <template v-slot="{ row }">
                {{ row.secondWeekFact ? kpiFormatter(row.secondWeekFact,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="15-21日计划" align="right" width="85" field="thirdWeekFact">
              <template v-slot="{ row }">
                {{ row.thirdWeekFact ? kpiFormatter(row.thirdWeekFact,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="15-21日实际" align="right" width="85" field="thirdWeekFact">
              <template v-slot="{ row }">
                {{ row.thirdWeekFact ? kpiFormatter(row.thirdWeekFact,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="22-30日计划" align="right" width="85" field="fourthWeekTarget">
              <template v-slot="{ row }">
                {{ row.fourthWeekTarget ? kpiFormatter(row.fourthWeekTarget,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
            <vxe-table-column title="22-30日实际" align="right" width="85" field="fourthWeekFacg">
              <template v-slot="{ row }">
                {{ row.fourthWeekFacg ? kpiFormatter(row.fourthWeekFacg,row.quotaName) : '-' }}
              </template>
            </vxe-table-column>
          </vxe-table-column>
        </vxe-table>
  ...
  export default {
    ...
    methods: {
      ...
      cellClassName ({ row, rowIndex, column, columnIndex }) {
        ...
        if (['quotaBigType', 'quotaSmallType'].includes(column.property)){
          return 'ch-bg-gary'
        }

        if (['lastTarget'].includes(column.property)){
          return 'ch-bg-gary2'
        }

        if (['lastComplete'].includes(column.property)){
          return 'ch-bg-green'
        }
      }
    },
