    ...
    <div class="box" ref="pageContent">
       <el-row class="but-box" type="flex" align="middle">
        <el-col style="text-align: left">
          <span class="dot"></span>
          <span class="title_name">月度养户贷款计划汇总</span>
        </el-col>
...
import { getForNew, add, getTree, unAudit, checkReportIsSp, audit,getSubOrg} from '@/api/maintenanceLoan/index'
      ...
      const that = this;
        $('.box').delegate('.checkDetails', 'click', function() {
          if (that.orgId && that.period) {
            that.$router.push({ path: '/pagePenetrationSix', query: {orgId: that.orgId,yearMonth:that.period,type: that.type, pagehead:"conserveLoans"}});
          }else{
            that.$message({
              message: '请先选择组织与计划期间',
              type: "warning"
    ...
    methods:{
      ...
      searchData(params){
        getForNew(params).then(res =>{
          if (res.status == 200) {
            if (!res.data.grCustloanplanSumInfo && res.data.entryInfoList && res.data.entryInfoList.length > 0) {
              let obj = {
            ...
            }else if (res.data.grCustloanplanSumInfo){
              if ([1,2].includes(res.data.grCustloanplanSumInfo.status)){
                this.status = res.data.grCustloanplanSumInfo.status
                this.reverseAuditStatus = true
                this.isSumit = false
                if (this.status == 2) {
                  this.checkReport(res.data.grCustloanplanSumInfo)
                }
              } else {
                this.reverseAuditStatus = false
                this.isSumit = false
              }
              this.formData = res.data.grCustloanplanSumInfo
            }else{
              this.reverseAuditStatus = false
              this.formData = {}
            }
        ...
        if (columnIndex == 0) {
          if (row.orgLongNumber) {
            if(parseInt(this.type) == 1){
              this.$router.push({ path: '/planBalanceReport/servicePlan/conserveLoans'+(parseInt(this.type)+2), query: {orgId: row.orgId,orgId2:this.orgId2,orgIdthree:this.orgIdthree,yearMonth:this.period,orgLongNum: row.orgLongNumber,type:parseInt(this.type)+2}});
            }else{
              this.$router.push({ path: '/planBalanceReport/servicePlan/conserveLoans'+(parseInt(this.type)+1), query: {orgId: row.orgId,orgId2:this.orgId2,orgIdthree:this.orgIdthree,yearMonth:this.period,orgLongNum: row.orgLongNumber,type:parseInt(this.type)+1}});
            }
            
          }else{
            this.$router.push({ path: '/servicePlan/conserveLoans', query: {orgId: row.orgId,yearMonth:this.period}});
          }
        }
      },
      // 反审核 审核
      auditEvent(){
        ...
        this.$confirm(tips, '提示', {
          ...
          if (this.status == 1) {
            let params = {
              entryInfoList: this.tableData,
              grCustloanplanSumInfo: this.formData
            }
            this.loading = true
      ...
      exportDataEvent(){
        if(this.formData.id){
            let myObj = {
              method: 'get',
              url:'custloanplansum/exportExcel',
              fileName: '月度养户贷款计划汇总表',
