    ...
    <div class="box" ref="pageContent">
       <el-row class="but-box" type="flex" align="middle">
        <el-col style="text-align: left">
          <span class="dot"></span>
          <span class="title_name">月度出苗计划汇总</span>
        </el-col>
...
import { getForNew, add, getTree, checkReportIsSp, unAudit} from '@/api/seedlingEmergence/index'
import $ from 'jquery'
...
      }
    ...
    mounted(){
      ...
      $('.box').delegate('.checkDetails', 'click', function() {
        if (that.orgId && that.period) {
          that.$router.push({ path: '/pagePenetrationThree', query: {orgId: that.orgId,yearMonth:that.period,type: that.type, pagehead:"seedlingEmergence"}});
        }else{
          that.$message({
            message: '请先选择组织与计划期间',
            type: "warning"
    ...
    methods:{
      ...
      submitData(state){
        ...
        obj.status = state
        obj.sproutPlanTotalEntryList = null
        obj.sproutPlanTotalEntryList = this.tableData
        this.loading = true
        add(obj).then(res =>{
          if (res.status == 200) {
            this.formData = res.data.parent
        ...
        if (columnIndex == 0) {
          if (!row.type) {
            if(parseInt(this.type) == 1){
              this.$router.push({ path: '/planBalanceReport/pigPlan/seedlingEmergence'+(parseInt(this.type)+2), query: {orgId: row.orgId,yearMonth:this.period,type: parseInt(this.type)+2}});
            }else{
              this.$router.push({ path: '/planBalanceReport/pigPlan/seedlingEmergence'+(parseInt(this.type)+1), query: {orgId: row.orgId,yearMonth:this.period,type: parseInt(this.type)+1}});
            }
          }else{
            this.$router.push({ path: '/pigFarmPlan/seedlingPlanDetails', query: {orgId: row.orgId,yearMonth:this.period}});
          }
        }
      },
      // 反审核  审核
      ...
      exportDataEvent(){
        ...
        let myObj = {
          method: 'get',
          url:'sproutPlanTotal/exportTotalByOrgAndPeriod',
          fileName: '月度出苗计划汇总表',
          params: params
        }
        exportMethods(myObj)
